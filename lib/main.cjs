...
class Pinole extends EventEmitter {
  ...
  write (data = '') {
    ...
    if (this.writing) {
      this.cache += data
      // TODO figure out a good size for emergency flush sync
  ...
  flushSync () {
    ...
    // todo: fuck this off in favour of a async waiter promise for sync mode,
    // in a race vs this message
    this.worker.postMessage(null)
    // clear any on going async writes
    ...
    Atomics.store(this.meta, MODE, ASYNC)
    Atomics.notify(this.meta, MODE)

    Atomics.store(this.meta, STATUS, READY)
    Atomics.notify(this.meta, STATUS)
